      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex items-start justify-between gap-3">
          ...
          <div className="relative">
            <button type="button" className="rounded-[10px] p-1 text-stone-400 opacity-0 transition-opacity hover:bg-stone-100 hover:text-stone-900 group-hover:opacity-100" onClick={() => setOptionsOpen((open) => !open)} aria-label={`More options for ${post.id}`} data-testid={`button-post-options-${post.id}`}><MoreHorizontal className="h-4 w-4" /></button>
            {optionsOpen && <div className="absolute right-0 top-8 z-10 w-44 rounded-[10px] border border-stone-200 bg-white p-3 text-[11px] leading-relaxed text-stone-500" data-testid={`panel-post-options-${post.id}`}>Post details are read-only in this workspace.</div>}
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2 text-[11px] font-medium text-stone-500">
          ...
          <span className={`ml-auto rounded-full px-2.5 py-1 text-[10px] font-bold ${post.status === 'Scheduled' ? 'bg-stone-100 text-stone-600' : 'bg-[#C2410C]/10 text-[#C2410C]'}`} data-testid={`status-post-${post.id}`}>{post.status}</span>
        </div>
      </div>
    </article>
  );
