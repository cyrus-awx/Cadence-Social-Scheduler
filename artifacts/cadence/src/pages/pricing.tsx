  ...
  return (
    <div className="mx-auto max-w-[1120px]">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {plans.map((plan) => (
          <article key={plan.name} className={`relative flex flex-col rounded-[10px] p-5 sm:p-7 ${plan.tone === 'featured' ? 'border-2 border-[#C2410C] bg-[#C2410C] text-white' : 'border border-stone-200 bg-white text-stone-900'}`} data-testid={`card-plan-${plan.name.toLowerCase()}`}>
